...
def aide_transformice(s, pseudo, canal, message):
    ...
    s.send("NOTICE",pseudo,":\x02!omodo\x02 pour avoir la liste des \x02modérateurs\x02 et \x02administrateurs\x02 connectés en jeu.")
...
def extra_cmd(s, pseudo, canal, message):
    ...
    s.send("NOTICE",pseudo,":!statwot <pseudo d'un joueur> pour avoir les statistiques du joueur (sur World of Tanks).")
    ...
    if len(citations.names) == 0:
        s.send("NOTICE",pseudo, ":!citation <theme>(optionnel) pour obtenir une citation au hasard. Thèmes disponibles: aucun thème n'est disponible.")
    else:
        s.send("NOTICE",pseudo, ":!citation <theme>(optionnel) pour obtenir une citation au hasard. Thèmes disponibles: "+" ".join(citations.names))
    ...
    if len(message) > 1:
        try:
            ...
            if nickname == message[1]:
                to_send = ":" + nickname + " a participé à " + str(nb_battles) + " battailles! V: "+str(nb_wins)+" - frags: "+str(nb_frags)+" - max frag: "+str(max_frags)+" - max dmg: "+str(max_dmg)
        except:
            to_send = ":Joueur inconnu!"
        if canal[0] == '#':
...
def send_tank(s, pseudo, canal, message):
    ...
    try:
        real_name, nation, type_ = wot.get_info_tank(tank_name)
        to_send = ":" + real_name + " est un " + type_ + " de " + nation + "."
    except:
...
def send_pizza(s, pseudo, canal, message):
    if canal[0] == '#':
        s.send("PRIVMSG", canal, ":ACTION envoie une pizza à la figure de " + pseudo + "")
    else:
        s.send("PRIVMSG", pseudo, ":ACTION envoie une pizza à la figure de " + pseudo + "")
...
def send_weather(s, pseudo, canal, message):
    ...
    try:
        real_name, description, temp, humidity, pression, country = meteo.get_weather(location)
        to_send = ":" + real_name + "(" + country + "), temps " + description.lower() + " - " + str(temp) + "°C - " + str(humidity) + "% d'humidité - pression atmosphérique de " + str(pression) + "hpa"
    except:
